<script>
    function get_rep_cmt($id){
        ...
        $.ajax({
            type:'GET',
            url:'<?php echo e(route("get-rcomment-story-chapter")); ?>',
            data:
            {
    ...
    function repBinhLuan($id){
        ...
        {    $.ajax({
                type:'POST',
                url:'<?php echo e(route("xu-ly-rep-binh-luan-chapter")); ?>',
                data:
                {
                _token : '<?php echo csrf_token() ?>',
                comment_chapter_id: $id,
    ...
    <li class="list-group-item">
        <article class="row">
            <div class="col-md-2 col-sm-2 col-xs-3 padding-mobile-cmt" style="text-align: center;">
                ...
                <?php endif; ?>
                <input onclick="rep_comment(<?php echo e($comment->id); ?>)" id="rep_comment<?php echo e($comment->id); ?>" type="button" value="Trả lời">
            </div>
            <div class="col-md-10 col-sm-10 col-xs-9 padding-mobile-cmt">
                <div class="panel panel-default arrow left">
                    ...
                    <?php if($comment->replay_comment_chapter->count() > 0): ?>
                    <div class="panel-footer ds-list-cmt" style="padding: 5px 0px;">
                        <ul class="list-group list-group-flush list-rep-cmt<?php echo e($comment->id); ?>">
                            <?php foreach($comment->replay_comment_chapter->reverse() as $key => $r_comment): ?>
                                <?php if($comment->replay_comment_chapter->count() - $key < 5): ?>
                        ...
                        <div class="more_r_comment<?php echo e($r_comment->comment_chapter_id); ?>" style="display: flex; justify-content: center; margin-top: -15px; display:none">
                            <input id="more_r_comment<?php echo e($r_comment->comment_chapter_id); ?>" type="hidden" value=4>
                            <a onclick="get_rep_cmt(<?php echo e($r_comment->comment_chapter_id); ?>)" type="button" >Xem thêm</a>
                        </div>
                        <?php if($comment->replay_comment_chapter->count() > 4): ?>
                        <script>
                            $('.more_r_comment<?php echo e($r_comment->comment_chapter_id); ?>').css("display","flex");
                        </script>
                        <?php endif; ?>
                    </div>
                    <?php else: ?>
