...
@section('seo')
    <meta name="description"
        content="{{ $chapter->subname }}: {{ $chapter->name }}. N&#7897;i dung: {!! substr(tanvo($chapter->content), 0, 230) !!}" />
    ...
    <div class="container chapter" id="chapterBody" style="margin-top: 0px; overflow-x: hidden;">
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <br><br>
                <a class="truyen-title" href="{{ route('story.show', $story->alias) }}"
                    title="{{ $story->name }}">{{ $story->name }}</a>
                {{--    <div class="col-lg-12 col-12" style="margin-bottom: 16px"> --}}
                {{--        <button onclick="readText()">Đọc Truyện</button> --}}
                {{--    </div> --}}
                <h2>
                    <a class="chapter-title" href="{{ route('chapter.show', [$story->alias, $chapter->alias]) }}"
                ...
                <div class="chapter-content" id="dynamic-content">
                    ...
                    {{-- Hiện popup chapter thông báo ở đây --}}
                    <input type="hidden" id="popup_value" value="{{ $popup_alert_chapter != null ? 1 : 0 }}">
                    @if ($popup_alert_chapter != null)
                        <div class="modal fade" data="popup_alert_chapter" id="popup_alert_chapter" tabindex="-1"
                            role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLongTitle">{{ $popup_alert_chapter }}</h5>
                    ...
                    @endif
                    {{-- Hiện popup coccoc quảng cáo ở đây --}}
                    <input type="hidden" id="popup_coccoc_check" value="{{ $popup_coccoc == true ? '1' : '0' }}">
                    ...
                    @if ((Auth::check() && $chapterNumber <= $chapter_vip) || (Auth::check() && $chapter_vip == null))
                        {!! $chapter->content !!}
                    @endif
                    @if (!Auth::check() && $chapterNumber >= $chapter_limit && $chapter_limit == '')
                        {!! $chapter->content !!}
                    @endif
                    @if (!Auth::check() && $chapterNumber <= $chapter_limit && $chapter_limit != '')
                        {!! $chapter->content !!}
                    ...
                    @endif
                    {{-- checkvip --}}
                    @if (Auth::check() && $chapterNumber > $chapter_vip && $chapter_vip != null)
                ...
                <div class="chapter-nav" id="chapter-nav-bot">
                    ...
                    <div class="text-center">
                        <button type="button" class="btn btn-warning" id="chapter_error"
                            chapter-id="{{ $chapter->id }}"><span class="glyphicon glyphicon-exclamation-sign"></span>
                            Báo lỗi chương</button>
                        <button type="button" class="btn btn-info" onclick="showtt('fb','tt')" id="chapter_comment"
                            chapter-id="{{ $chapter->id }}"><span class="glyphicon glyphicon-comment"></span>
                            Tieuthuyet.vn({{ $chapter->comment_chapter->count() }})</button>
                        <button type="button" class="btn btn-info" onclick="showfb('fb','tt')"
                            id="chapter_comment"chapter-id="{{ $chapter->id }}"><span
                    ...
                    <div class="col-xs-12">
                        <div id="fb" class="fb-comments"
                            data-href="{{ route('chapter.show', [$story->alias, $chapter->alias]) }}" data-width="100%"
            ...
            ?>
            <div style="padding:0px" class="col-xs-12" id="truyen-slide">
                <div class="list list-thumbnail" id="list-category">
                    ...
                    <div class="row">
                        @foreach ($getListForCategory as $storyForCategory)
                            <div class="col-xs-4 col-sm-4 col-md-2 list-category" style="height: auto ; border:0px">
