...
function tanvo($string) {

    $string = str_replace(array('<p>', '</p>', '<br>', '<br />','<br/>'), ' ', $string);
    $string = preg_replace('/(<.*?>)|(<span>.*?<\/span>)|(<span .*?>.*?<\/span>)|(<em>.*?<\/em>)|(<em .*?>.*?<\/em>)/', '', $string);
    $string = preg_replace('/(<p .*?>)|(<\/p>)|(<br>)|(<br \/>)|(<span>.*?<\/span>)|(<span .*?>.*?<\/span>)|(<em>.*?<\/em>)|(<em .*?>.*?<\/em>)/', '', $string);
...
//             foreach ($data as $d)
//             {
//                 $item[] = '"'.route('author.list.index', $d->alias).'"';
//             }
//         }
//         else
//         {
//             foreach ($data as $d)
//             {
//                 $item[] = '"'.route('category.list.index', $d->alias).'"';
//             }
//         }
//     }
//     else
...
function get_author($data, $limit = ', ')
{
    $item = [];
    if ($data)
    {
        foreach ($data as $d)
        {
            $item[] = '{
                            "@type": "Person",
                            "name": "'.$d->name.'",
                            "url": "'.route('author.list.index', $d->alias).'"
                        }';
        }
    }
    else
...
function get_catory($data, $limit = ', ')
{
    $item = [];
    if ($data)
    {
        foreach ($data as $d)
        {
            $item[] = '{
                            "@type": "Thing",
                            "name": "'.$d->name.'",
                            "url": "'.route('category.list.index', $d->alias).'"
                        }';
        }
    }
    else
...
function showBreadcrumb($data = null)
{
    ...
    $result = $result . '<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                            <a href="' . url('/') . '" accesskey="1">
                                <span class="glyphicon glyphicon-home"></span>
                            </a>
                            <a itemprop="item" href="' . url('/') . '" title="Đọc truyện online">
    ...
    if (!is_null($data)) foreach($data as $d)
        $result = $result . '<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                                <a itemprop="item" href="' . $d[0] . '" title="' . $d[1] . '">
                                    <span itemprop="name">' . $d[1] . '</span>
                                </a>
